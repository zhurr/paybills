    ...
    export default {
        ...
        data() {
            return {
                ...
                filter: [{ name: '裸金属', label: '物理机' }, { name: '云主机', label: '虚拟机' }],
                activeFilter: '物理机',
        ...
        methods: {
            ...
            getMaxDatas() {
                ...
                this.$axios.post('/v1/cmdb/index/utilization/monthContrast/max', params).then((res) => {
                    this.gaugeDatas[0].chartDatas.rate = fixedNumber(res.data.current.cpuMax, 2);
                    this.gaugeDatas[0].chartDatas.change = subNumber(res.data.current.cpuMax, res.data.previous.cpuMax, 2)
                    this.gaugeDatas[2].chartDatas.rate = fixedNumber(res.data.current.storeMax, 2);
                    this.gaugeDatas[2].chartDatas.change = subNumber(res.data.current.storeMax, res.data.previous.storeMax, 2)
            ...
            getAvgDatas() {
                ...
                this.$axios.post('/v1/cmdb/index/utilization/monthContrast/avg', params).then((res) => {
                    this.gaugeDatas[1].chartDatas.rate = fixedNumber(res.data.current.cpuAvg, 2);
                    this.gaugeDatas[1].chartDatas.change = subNumber(res.data.current.cpuAvg, res.data.previous.cpuAvg, 2);
                    this.gaugeDatas[3].chartDatas.rate = fixedNumber(res.data.current.storeAvg, 2);
                    this.gaugeDatas[3].chartDatas.change = subNumber(res.data.current.storeAvg, res.data.previous.storeAvg, 2);

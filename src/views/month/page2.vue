                ...
                <div class="left"
                    ...
                    <div class="chart-wrap"
                        ...
                        <section class="chart-item"
                                 ...
                                 :key="item.deviceType">
                            <p class="title">{{item.deviceTypeObj.name}}{{item.moduleType}}</p>
                            <div class="gaugle-wrap">
                                <p class="gaugle-num text-ellipse"
                                   :title="item.amount">{{item.amount}}</p>
                                <p class="gaugle-name">{{item.deviceTypeObj.name}}已分配({{item.unit}})</p>
                            ...
                            <section class="progress-wrap">
                                <YwProgressSingle :progressDatas="{percent:item.percent,name:'未部署采集点数量',num:`${item.percent}%，${item.undeploy}个`}">
                                </YwProgressSingle>
                            </section>
...
<script>
    ...
    export default {
        ...
        components: {
            ...
            page2_table: () => import('./page2-table.vue'),
                ...
                this.$axios.post('/v1/cmdb/index/allocate/resource', params).then((res) => {
                    this.leftDatas = res.data.calculate.map((item, index) => {
                        ...
                        switch (item.deviceType) {
                            case '物理机':
                                deviceTypeObj = { code: '物理机', name: '裸金属' }
                                break
                            case '虚拟机':
                                deviceTypeObj = { code: '虚拟机', name: '云主机' }
                                break
                            default:
                                deviceTypeObj = { code: item.deviceType, name: item.deviceType }
                                break
                        }
                        return {
                            amount: item.amount,

...
<script>
    ...
    export default {
        name: 'page4',
        mixins: [DrawChart, ShowEmptyDatas, QueryObject, updateComponent],
        components: {
        ...
        data() {
            return {
                ...
                tableTitles: [],
                tableDatas: [],
                chartDatas: [{ id: 'page4-1-1', chartOption: 'pie-option', chartDatas: [], typeObj: { count: 0, name: '裸金属CPU及内存', type: { code: '物理机', name: '裸金属' }, desc: '峰值利用率低于30%', moduleFlag: 0 }, active: true },
                { id: 'page4-1-2', chartOption: 'pie-option', chartDatas: [], typeObj: { count: 0, name: '云主机CPU', type: { code: '虚拟机', name: '云主机' }, desc: '峰值利用率低于30%', moduleFlag: 1 }, active: false },
                { id: 'page4-1-3', chartOption: 'pie-option', chartDatas: [], typeObj: { count: 0, name: '云主机内存', type: { code: '虚拟机', name: '云主机' }, desc: '峰值利用率低于30%', moduleFlag: 2 }, active: false },
                { id: 'page4-1-4', chartOption: 'pie-option', chartDatas: [], typeObj: { count: 0, name: '云存储', type: { code: '云存储', name: '云存储' }, desc: '资源利用率低于40%', moduleFlag: 3 }, active: false }]
            }
        },
        ...
        methods: {
            ...
            getTableDatas() {
                ...
                this.tableTitles = [
                    ...
                    { name: 'cpuMax', title: `${this.activeModule.typeObj.type.name}CPU峰值利用率(%)`, style: { sortable: true, align: 'right', width: 220, 'text-link': true } },
                    { name: 'storeMax', title: `${this.activeModule.typeObj.type.name}内存峰值利用率(%)`, style: { sortable: true, align: 'right', width: 220, 'text-link': true } },
                ];

                if (this.activeModule.typeObj.type.code === '云存储') {
                    this.tableTitles = [
                        { name: 'bizSystem', title: `应用系统 (${this.activeModule.typeObj.name}${this.activeModule.typeObj.desc})`, style: {} },
                        { name: 'resourcePool', title: `所属资源池`, style: {} },
                        { name: 'cloudStore', title: `存储利用率(%)`, style: { sortable: true, align: 'right', width: 220, 'text-link': true } },
                    ];
                ...
                this.$axios.post('/v1/cmdb/index/bisSystem/list/cal', params).then((res) => {
                    this.tableDatas = res.data.map((item) => {
                        return {
                            ...
                            storeMax: fixedNumber(item.storeMax, 2)
                        }
                    });
                });
            },
                ...
                this.$axios.post('/v1/cmdb/index/bisSystem/list/store', params).then((res) => {
                    this.tableDatas = res.data.map((item) => {
                        return {
                            bizSystem: item.bizSystem,
                            resourcePool: item.resourcePool,
                            cloudStore: fixedNumber(item.cloudStore, 2)
                        }
                    });
                });
            },
